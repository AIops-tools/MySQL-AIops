...
### Changed
- **The ClawHub bundle plugin moved from `@aiops-tools/mysql-aiops` to
  `@zw008/mysql-aiops`**, matching the publisher the skill has always been under.
  ...
  `openclaw plugins install clawhub:@zw008/mysql-aiops`. Nothing about the Python
...
- **The OpenClaw install path is documented.** The ClawHub bundle channel went
  ...
  `openclaw plugins install clawhub:@aiops-tools/mysql-aiops`. States the `uvx`
...
- **Installable as a Claude Code plugin.** `.claude-plugin/plugin.json` plus a
  root `.mcp.json` make this repo a plugin, so `/plugin install mysql-aiops@aiops-tools`
...
- **Installable from ClawHub as an OpenClaw bundle plugin** (`@aiops-tools/mysql-aiops`): one install delivers the skill *and* its MCP
...
- **The skill was invisible to the model in OpenClaw.** Its metadata
  ...
  `anyBins: [mysql-aiops, uvx]` — either one suffices — with every variable kept
...
- **`as_int` rejects a bool rather than coercing it.** `bool` subclasses `int`, so a boolean would otherwise have been reported as a count. The conversion itself was already exact (no float round-trip), unlike the sibling copies fixed in the same sweep.
