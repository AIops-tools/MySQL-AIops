"""``mysql-aiops secret`` — manage the encrypted credential store.

Secrets (MySQL account passwords) are stored in ``~/.mysql-aiops/secrets.enc``
...
from mysql_aiops.config import SECRET_ENV_PREFIX, SECRET_ENV_SUFFIX
...
def secret_set(
    ...
    """Store (or replace) the password for a target — value is read hidden."""
...
def secret_list() -> None:
    ...
    names = store.names()
    if not names:
        console.print("[yellow]No secrets stored yet. Add one: mysql-aiops secret set <name>[/]")
        return
...
def secret_rotate_password() -> None:
    ...
    console.print("[green]✓ Master password rotated. Update MYSQL_AIOPS_MASTER_PASSWORD.[/]")
