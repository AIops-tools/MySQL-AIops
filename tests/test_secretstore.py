...
nothing touches the real ``~/.mysql-aiops``.
"""
...
def test_migrate_legacy_env(store_dir):
    (store_dir / ".env").write_text(
        "MYSQL_PRIMARY_PASSWORD=legacy-pass\n# comment\nFOO=bar\n"
    )
    imported = ss.migrate_legacy_env("MYSQL_", "_PASSWORD", "pw")
    assert imported == ["primary"]
    assert ss.SecretStore.unlock("pw").get("primary") == "legacy-pass"
