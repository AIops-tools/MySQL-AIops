...
throwaway home (``MYSQL_AIOPS_HOME``) and assert that the rows compliance
evidence is built from actually land on disk. The harness authorizes nothing:
a high-risk write runs and is audited whether or not an approver annotation
is set.
"""
...
from mysql_aiops.governance import (
    capture_prior_state,
    governed_tool,
    mark_unknown,
)
...
def gov_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("MYSQL_AUDIT_APPROVED_BY", raising=False)
    _reset_singletons()
    yield tmp_path
    _reset_singletons()
...
def test_medium_write_persists_audit_and_undo_rows(gov_home):
    result = _rename_widget(name="w1", target="prod-db")
...
def test_high_risk_runs_without_an_approver_and_is_audited(gov_home):
    ...
    result = _drop_widget(name="w2", target="prod-db")
...
def test_an_approver_is_still_recorded_when_supplied(gov_home, monkeypatch):
    ...
    monkeypatch.setenv("MYSQL_AUDIT_APPROVED_BY", "dba-alice")
    result = _drop_widget(name="w3", target="prod-db")
    assert result["status"] == "dropped"

    audit = _rows(gov_home / "audit.db", "audit_log")
    assert audit[0]["status"] == "ok"
    assert audit[0]["approved_by"] == "dba-alice"


@pytest.mark.unit
def test_real_write_tool_persists_priorstate_undo(gov_home, monkeypatch, fake_mysql):
    """End-to-end through a REAL product write tool: drop_index must capture
    the index definition (from SHOW CREATE TABLE) before dropping and persist
    the recreate-undo on disk."""
    monkeypatch.setenv("MYSQL_AUDIT_APPROVED_BY", "dba-alice")
    ddl = (
        "CREATE TABLE `orders` (\n"
        "  `id` bigint NOT NULL,\n"
        ...
        "  PRIMARY KEY (`id`),\n"
        "  KEY `idx_orders_ts` (`ts`)\n"
        ") ENGINE=InnoDB"
    )
    fake = fake_mysql({"SHOW CREATE TABLE": [{"Table": "orders", "Create Table": ddl}]})

    from mcp_server.tools import remediation as gov

    monkeypatch.setattr(gov, "_get_connection", lambda target=None: fake)
    result = gov.drop_index(table="orders", name="idx_orders_ts")
    expected_def = "CREATE INDEX `idx_orders_ts` ON `orders` (`ts`)"
    assert result["priorState"]["definition"] == expected_def
    assert result.get("_undo_id")

    undo = _rows(gov_home / "undo.db", "undo_log")
    assert len(undo) == 1
    assert expected_def in undo[0]["undo_params"]
