...
from mysql_aiops.governance import READ_ONLY_ENV, is_read_only
...
def test_apply_read_only_unregisters_write_tools(monkeypatch):
    ...
    try:
        ...
        assert "drop_index" in dropped and "kill_query" in dropped
        assert "top_queries" not in dropped and "table_sizes" not in dropped
        ...
        assert "top_queries" in remaining, "reads must still be exposed"
    finally:
        registry.clear()
