...
2. **Runaway breaker (on by default).** The same ``(tool, params)`` called more
   than ``MYSQL_RUNAWAY_MAX`` times within ``MYSQL_RUNAWAY_WINDOW_SEC`` trips a
...
from mysql_aiops.governance.policy import PolicyResult
...
class BudgetExceeded(Exception):
    ...
    Subclasses nothing MySQL-specific; the decorator wraps it into a
