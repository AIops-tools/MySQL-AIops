...
the SAME governance path as any other call (audited, budget-checked, and
carrying its own risk tier into the audit row). ``undo_apply`` is itself
governed; the inner tool it calls is the one that actually runs.
...
def undo_apply(undo_id: str, dry_run: bool = False, target: Optional[str] = None) -> dict:
    ...
    The inverse runs through its own governed tool, so its own risk tier and
    audit row are recorded there. Pass dry_run=True to preview
