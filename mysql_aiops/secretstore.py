"""Encrypted secret storage for mysql-aiops.

Secrets (MySQL account passwords) are NEVER written to disk in plaintext. They
live in ``~/.mysql-aiops/secrets.enc`` encrypted with Fernet (AES-128-CBC +
...
The master password is resolved (in order) from:
  ...
  1. the env var ``MYSQL_AIOPS_MASTER_PASSWORD`` (for non-interactive use:
...
APP_NAME = "mysql-aiops"
CONFIG_DIR = ops_home()
MASTER_PASSWORD_ENV = "MYSQL_AIOPS_MASTER_PASSWORD"  # nosec B105 — env-var name
# ──────────────────────────────────────────────────────────────────────────────
...
def migrate_legacy_env(prefix: str, suffix: str, password: str | None = None) -> list[str]:
    ...
    Legacy keys look like ``MYSQL_<TARGET>_PASSWORD``; ``prefix='MYSQL_'`` and
